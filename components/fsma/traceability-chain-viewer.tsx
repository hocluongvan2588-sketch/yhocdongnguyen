      ...
      <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
        ...
        <div className="w-[calc(100%-4rem)] md:w-[45%] p-4 rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between space-x-2 mb-1">
            ...
            <time className="font-mono text-xs font-medium text-indigo-500">{event.date}</time>
          </div>
...
function ChainNode({ node, isRoot = false, t }: { node: TraceabilityNode, isRoot?: boolean, t: (key: string) => string }) {
  ...
  return (
    <div className={`flex items-start gap-4 p-4 border rounded-xl bg-white transition-all ${isRoot ? 'border-indigo-500 ring-2 ring-indigo-50 shadow-md' : 'border-gray-200 hover:border-indigo-300 shadow-sm'}`}>
      <div className="flex-1 space-y-3">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h4 className="font-bold font-mono text-gray-900">{node.lot_code}</h4>
              {isRoot && <Badge className="bg-indigo-600 hover:bg-indigo-700 text-[10px] uppercase">Gốc</Badge>}
            </div>
            <p className="text-xs text-gray-500">{node.product_description}</p>
          </div>
          <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${eventTypeColors[node.event_type] || "bg-gray-100"}`}>
            {t(`cte.${node.event_type}`)}
          </span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[11px]">
          <div>
            <span className="text-gray-400 block uppercase font-semibold">Số lượng</span>
            <p className="font-bold text-gray-700">{node.quantity} {node.unit_of_measure}</p>
          </div>
          <div>
            <span className="text-gray-400 block uppercase font-semibold">Địa điểm</span>
            <p className="font-bold text-gray-700 truncate">{node.location_name || "N/A"}</p>
          </div>
          <div>
            <span className="text-gray-400 block uppercase font-semibold">Ngày ghi nhận</span>
            <p className="font-bold text-gray-700">
              {new Date(node.event_datetime).toLocaleDateString('vi-VN')}
            </p>
          </div>
          <div>
            <span className="text-gray-400 block uppercase font-semibold">Trạng thái</span>
            <p className="font-bold text-gray-700 capitalize text-green-600">{node.status}</p>
          </div>
        </div>
      </div>
    </div>
...
function LossAnalysis({ backward, current, forward }: { backward: TraceabilityNode[], current: TraceabilityNode, forward: TraceabilityNode[] }) {
    ...
    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5 mt-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h4 className="font-bold text-yellow-900 text-sm mb-3 uppercase tracking-tight">Phân tích thất thoát (Loss Analysis)</h4>
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-white/50 p-3 rounded-lg">
              <span className="text-[10px] text-yellow-700 uppercase font-bold">Số lượng đầu vào</span>
              <p className="text-lg font-black text-yellow-900">{initialQuantity} {current.unit_of_measure}</p>
            </div>
            <div className="bg-white/50 p-3 rounded-lg">
              <span className="text-[10px] text-yellow-700 uppercase font-bold">Tổng thất thoát</span>
              <p className="text-lg font-black text-red-600">{totalLoss.toFixed(2)} {current.unit_of_measure}</p>
            </div>
            <div className="bg-white/50 p-3 rounded-lg">
              <span className="text-[10px] text-yellow-700 uppercase font-bold">Tỷ lệ hao hụt</span>
              <p className="text-lg font-black text-red-600">{lossPercentage}%</p>
            </div>
          </div>
          <p className="text-[10px] text-yellow-600 mt-3 italic">* Dựa trên chênh lệch giữa nguyên liệu đầu vào và thành phẩm cuối cùng.</p>
        </div>
      </div>
    </div>
  )
...
export function TraceabilityChainViewer({ initialLotCode = "" }: TraceabilityChainViewerProps) {
  ...
  // Hàm dịch giả lập (Mocking t function)
  const t = (key: string) => {
  ...
  // Tự động tìm kiếm nếu có initialLotCode khi mount
  useEffect(() => {
    if (initialLotCode) {
      handleSearch()
    }
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden border-slate-200 shadow-sm">
        <CardHeader className="bg-slate-50/50 border-b border-slate-100">
          ...
          <CardDescription>Nhập mã lô hàng (Lot Code) để xem sơ đồ dòng chảy sản phẩm</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 space-y-2">
              <Label htmlFor="lotCodeInput">Mã lô sản phẩm (TLC)</Label>
            ...
            <Button onClick={handleSearch} disabled={isSearching} className="sm:self-end bg-indigo-600 hover:bg-indigo-700">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
          <Card className="border-slate-200">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                <CardTitle className="text-lg">Sơ đồ chuỗi giá trị</CardTitle>
                <CardDescription>Mã lô: <span className="font-mono font-bold text-indigo-600">{currentLot.lot_code}</span></CardDescription>
              </div>
          ...
          <Card className="border-slate-200">
            <CardHeader>
              <CardTitle className="text-lg">Dòng thời gian sự kiện</CardTitle>
              <CardDescription>Chi tiết các điểm kiểm soát tới hạn (CTE) được ghi nhận</CardDescription>
            </CardHeader>
            <CardContent>
              <CTETimeline 
      ...
      {!hasData && !isSearching && (
        <Card className="border-dashed border-2 bg-slate-50/50">
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-slate-500 font-medium">Nhập mã lô hàng để bắt đầu hành trình truy xuất</p>
          </CardContent>
        </Card>
      )}
    </div>
